...
FirePHPChrome.BottomToolbar.BrowserWatcher =
{    
...
//            if (!persistedState || persistedState.location != win.location.href)
            ...
            context = this.owner.createTabContext(win, browser, ((browser)?browser.chrome:null), persistedState);
    ...
    watchLoadedTopWindow: function(win)
    {
        ...
        {
alert('watchLoadedTopWindow - unwatch window');        
    ...
    {
        if (!context)
            context = this.getContextByWindow(FirebugLib.getRootWindow(win));
    ...
    {
			browser.addProgressListener(FirePHPChrome.BottomToolbar.BrowserWatcher.FrameProgressListener, NOTIFY_STATE_DOCUMENT);
...
//        context.browser.persistedState = persistedState;
        ...
        FirebugLib.iterateWindows(context.window, function(win)
        {
            FirePHPChrome.BottomToolbar.BrowserWatcher.dispatch("unwatchWindow", [context, win]);
        });
...
//        fbs.countContext(false);
        ...
        FirebugLib.remove(contexts, context);
    },
...
FirePHPChrome.BottomToolbar.BrowserWatcher.BaseProgressListener =
{
...
FirePHPChrome.BottomToolbar.BrowserWatcher.TabProgressListener = FirePHPLib.extend(FirePHPChrome.BottomToolbar.BrowserWatcher.BaseProgressListener,
{
    onLocationChange: function(progress, request, location)
    {
        // Only watch windows that are their own parent - e.g. not frames
        if (progress.DOMWindow.parent == progress.DOMWindow)
            FirePHPChrome.BottomToolbar.BrowserWatcher.watchTopWindow(progress.DOMWindow, location);
    },
    ...
    onStateChange: function(progress, request, flag, status)
    {
        /*if (flag & STATE_STOP)
        {
            ...
            if (win && win.parent == win)
                BrowserWatcher.watchLoadedTopWindow(progress.DOMWindow);
        }*/
    }
});
...
FirePHPChrome.BottomToolbar.BrowserWatcher.FrameProgressListener = FirePHPLib.extend(FirePHPChrome.BottomToolbar.BrowserWatcher.BaseProgressListener,
{
    onStateChange: function(progress, request, flag, status)
    {
        ...
        if (flag & STATE_IS_REQUEST && flag & STATE_START)
        {
            if (safeGetName(request) == dummyURI)
            {
                ...
                if (win.parent == win && win.location.href == "about:blank")
                    FirePHPChrome.BottomToolbar.BrowserWatcher.watchTopWindow(win, null);

                FirePHPChrome.BottomToolbar.BrowserWatcher.watchWindow(win);
            }
        }
        ...
        if (flag & STATE_IS_DOCUMENT && flag & STATE_TRANSFERRING)
            FirePHPChrome.BottomToolbar.BrowserWatcher.watchWindow(progress.DOMWindow);
    }
});
...
{
	/* Dispable the unload for now */
//    FirePHPChrome.BottomToolbar.BrowserWatcher.unwatchTopWindow(event.currentTarget);
}
...
function onLoadWindowContent(event)
{
    ...
    setTimeout(function()
    {
        FirePHPChrome.BottomToolbar.BrowserWatcher.watchLoadedTopWindow(win);
    });
}
...
function onUnloadWindow(event)
{
    ...
    FirePHPChrome.BottomToolbar.BrowserWatcher.unwatchWindow(win);
}
